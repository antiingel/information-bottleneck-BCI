...
class ModelTrainer(object):
    ...
    def setup(self, look_back_length, feature_names, data, labels):
        ...
        self.t_matrix_builder_types = [True, False] #[False]
    ...
    def classify(self, intervals_all, intervals_all_inf, result_full_label, features):
        f1, f2, f3 = features
        for i, ((l1, h1), (l2, h2), (l3, h3)) in enumerate(intervals_all_inf):
            if l1 <= f1 <= h1 and l2 <= f2 <= h2 and l3 <= f3 <= h3:
                return result_full_label[i]

    def classifynew(self, intervals_inf, result_full_label, features):
        idxs = []
        for i, f in enumerate(features):
            for i, (l, h) in enumerate(intervals_inf[i]):
                if l <= f <= h:
                    idxs.append(i)
                    break
        return result_full_label[idxs[0]][idxs[1]][idxs[2]]

    def classify3new(self, intervals_inf, result_full_label, shift, threshold,  features):
        idxs = []
        for i, f in enumerate(features):
            for i, (l, h) in enumerate(intervals_inf[i]):
                if l <= f <= h:
        ...
        if shift == 0:
            ...
            labels = []
            for s1 in range(-shift, shift+1):
                for s2 in range(-shift, shift+1):
                    for s3 in range(-shift, shift+1):
                        j1 = idxs[0]+s1
                        j2 = idxs[1]+s2
                        j3 = idxs[2]+s3
                        if j1 < 0: j1 = 0
                        if j2 < 0: j2 = 0
                        if j3 < 0: j3 = 0
                        if j1 > result_full_label.shape[0]-1: j1 = result_full_label.shape[0]-1
                        if j2 > result_full_label.shape[1]-1: j2 = result_full_label.shape[1]-1
                        if j3 > result_full_label.shape[2]-1: j3 = result_full_label.shape[2]-1
                        if 0 <= j1 < result_full_label.shape[0] and 0 <= j2 < result_full_label.shape[1] and 0 <= j3 < result_full_label.shape[2]:
                            labels.append(result_full_label[j1][j2][j3])
    ...
    def start(self, subject, make_plots):
        ...
        for test_data_index in range(len(split_data)):
            ...
            n_classes = all_training_data.shape[0]
            ...
            for i in range(n_classes):
                ...
                for j in range(n_classes):
                    edges = np.histogram_bin_edges(all_training_data[i,:,j], "fd", (min_features_f[j], max_features_f[j]))
            ...
            if same_range:
                ...
                for j in range(n_classes):
                    ...
                    intervals_inf.append(
                        [(-float("inf"), bin_edges[j][0])] +
                        [(l,h) for l, h in zip(bin_edges[j][:-1],bin_edges[j][1:])] +
                        [(bin_edges[j][-1], float("inf"))]
                    )
                intervals = np.array(intervals)
                intervals_inf = np.array(intervals_inf)
                ...
                for i1, v1 in enumerate(intervals[0]):
                    for i2, v2 in enumerate(intervals[1]):
                        for i3, v3 in enumerate(intervals[2]):
                            intervals_all_inf.append((intervals_inf[0][i1], intervals_inf[1][i2], intervals_inf[2][i3]))
            ...
            for i, row in enumerate(discretised if skew else histogram):
                proba_row = []
                for i1, v1 in enumerate(row[0]):
                    for i2, v2 in enumerate(row[1]):
                        for i3, v3 in enumerate(row[2]):
                            proba_row.append(np.round(v1 * v2 * v3 / 3, 8))
            ...
            for beta in betas:
                ...
                for shift in [0,1,2,3,4]:
                    for thresh in range(*thresh_ranges[shift]):
                        ...
                        predicted_testing_labels = map(lambda x: self.classify3new(intervals_inf, reshape_result_full_label+1, shift, thresh, x), testing_prediction)

                        training_confusion_matrices += training_confusion_matrix[:,best_perm]
                        testing_confusion_matrix = sklearn.metrics.confusion_matrix(testing_labels,
